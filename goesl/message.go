...
// and dumping its contents. In addition to that it's to make sure received message is in fact message can be supported
...
// String returns message representation as string
func (m *Message) String() string {
	return fmt.Sprintf("%v body=%s", m.Headers, m.Body)
}

// GetCallUUID returns Caller-Unique-Id
func (m *Message) GetCallUUID() string {
	return m.GetHeader("Caller-Unique-Id")
}

// GetHeader returns message header value, or "" of the key is not set.
func (m *Message) GetHeader(key string) string {
	return m.Headers[key]
}

// Parse parses out message received from Freeswitch and builds it accordingly for later use.
func (m *Message) Parse() error {
	...
	Debug("Got message content (type: %s). ", msgType)
	...
	case "text/event-json":
		// What is missing here is a way to interpret other JSON types - it expects string only (understandably
		// because the FreeSwitch events are generally "string: string") - extract into empty interface and migrate
		// only strings. i.e. Event CHANNEL_EXECUTE_COMPLETE - "variable_DP_MATCH":["a=rtpmap:101 telephone-event/8000", "101"]
...
// newMessage builds and executes parsing against received freeswitch messsage.
